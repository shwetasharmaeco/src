...
def print_melon_at_price(price):
    ...
    If there are no melons at that price print "None found"
        ...
        None found
    """
    price_dict = {2.50 : ["Honeydew", "Cantaloupe"], 2.95 : ["Watermelon"], 3.25 : ["Musk", "Crenshaw"], 14.25 : ["Christmas"] }

    if price in price_dict:
        for melon in price_dict[price]:
            print (melon)
    else:
        print ("None found")
...
def kids_game(names):
    ...
    for name in names[1:]:
        names_dict[name[0]] = []
    for name in names[1:]:
        if name not in names_dict[name[0]]:
           names_dict[name[0]].append(name)        
    
    ans.append(names[0])  
   
    while ans[-1][-1] in names_dict:
        lst = names_dict.get(ans[-1][-1])
        candidate_val = lst[0]
        lst.remove(candidate_val)
        
        if not lst:
            names_dict.pop(ans[-1][-1], None)
        ans.append(candidate_val)    
    return ans    
